...
export default function EditButton({ onClick, className = "" }) {
  return (
    <button
      onClick={onClick}
      className={`font-semibold text-green-600 opacity-0 group-hover:opacity-100 duration-200 hover:text-green-500 hover:shadow-xs px-2 py-2 rounded-full transition-all cursor-pointer group-last ${className}`}
    >
      <svg className='transition-all duration-300 group-last:hover:scale-110' xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24">
